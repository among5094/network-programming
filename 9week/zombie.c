...
자식 프로세스의 PID (5957)는 커널의 프로세스 테이블에 남아있는 좀비 프로세스가 됨
---
자식 프로세스(PID 6499)가 종료되었지만,
부모 프로세스(PID 6498)가 자식의 종료 상태를 수거하지 않아서 좀비 상태로 남음.
부모 프로세스(PID 6498)를 종료하면, 운영체제는 자동으로 좀비 프로세스를 수거하여 제거함
